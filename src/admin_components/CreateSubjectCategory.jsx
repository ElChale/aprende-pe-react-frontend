...
import { createSubjectCategory } from '../reducers/subjectReducers'

function CreateSubjectCategory({ userInfo, categories }) {
      ...
      const handleSubmit = (e) => {
            e.preventDefault()
            dispatch(createSubjectCategory({ name:name, description:description, token:userInfo.token }))
            .then((result) => {
                  if (result.payload) {
      ...
      return (
            <div className='py-4 px-0 px-md-4'>
                  <h5 className='mb-3'>Crear categoria</h5>
