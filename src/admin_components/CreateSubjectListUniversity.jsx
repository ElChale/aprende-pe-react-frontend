...
import { createSubjectListUniversity } from '../reducers/subjectReducers'

function CreateSubjectListUniversity({ userInfo, universities, openCategory }) {
      ...
      const handleSubmit = (e) => {
            e.preventDefault()
            dispatch(createSubjectListUniversity({ subjects:subjects, category_id:openCategory.id, token:userInfo.token }))
            .then((result) => {
                  if (result.payload) {
      ...
      return (
            <div className='py-4 px-0 px-md-4'>
                  <h5 className='mb-3'>Crear cursos para la carrera: {openCategory?.name}</h5>
...
export default CreateSubjectListUniversity